...
## 大概workflow：
- 一开始输入自然文本，经过`tokenizer`处理后，得到索引`input_ids`。
- 经过`embedding`得到`hidden_states`对应的向量，对应token高维的解释。
- 经过`layers`之后再正则化，最后再线性化，得到`output`。

### layers：
- 先copy一个`hidden_states` 塞进残差网络
- 另一个`hidden_states`经过正则化，再计算自注意力，最后再加上经过残差的`hidden_states`
- 随后再重复上面操作，但把`attention`换成`MLP`，最后再加上经过残差的`hidden_states`
...
根据 `Qwen2Config`去建立`Qwen2Model`。\
手动写一个索引 `input_ids`, 在子词检索表中找到对应的词元。\
把索引 `input_ids` 喂给 `Qwen2Model`， 得到输出。
...
```python
# input_ids: (bs, seq_len) 字符串索引
# 将input_ids送入embed_tokens得到inputs_embeds
